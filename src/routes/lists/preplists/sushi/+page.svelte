<script lang="ts">
  ...
  const sortedItemIds = new Set(['pli-veg-sort-avocados', 'pli-veg-sort-mangos']);
  const isDone = (item: PreplistItem) => Number(item.is_checked) === 1;
  const isSortedCard = (item: PreplistItem) => sortedItemIds.has(item.id);
</script>
    ...
    <section class="sheet">
      ...
      <form id="prep-batch-form" method="POST" action="?/submit_prep_counts" class="batch-form">
        {#each data.items as item}
          <div class="sheet-row" class:done={isDone(item)} class:sorted-row={isSortedCard(item)}>
            <button
              type="submit"
              class="check-btn"
        ...
        <div class="actions-row">
          <button type="submit" class="submit-btn">{data.submitLabel ?? 'Submit Prep Counts'}</button>
        </div>
      </form>

      <div class="actions-row reset-row">
        <form method="POST" action="?/new_prep_list">
          <button type="submit" class="submit-btn subtle-btn">
            {data.resetLabel ?? 'New Prep List (Reset to 0)'}
          </button>
        </form>
      </div>
      ...
      {#if data.isAdmin}
        <details class="admin-par">
          <summary>{data.adminSummaryLabel ?? '+ Admin Par Levels'}</summary>
          <form method="POST" action="?/save_par_levels" class="admin-par-form">
            {#each data.items.filter((item) => !isSortedCard(item)) as item}
